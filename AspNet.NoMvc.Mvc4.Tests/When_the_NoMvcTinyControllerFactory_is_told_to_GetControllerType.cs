...
{
    public class When_the_NoMvcTinyControllerFactory_is_told_to_GetControllerType : NoMvcTest
    {
        [Fact]
        public void It_should_retrieve_the_controller_type_from_the_route_namespace()
        {
            ...
            var controllerType = mvcApplication.DefineControllerType(string.Format("{0}.Home", mvcApplicationName), "IndexController", new[] { "Get" });
            mvcApplication.DefineControllerType(string.Format("{0}.Home", mvcApplicationName), "FooController", new[] { "Get" });
            mvcApplication.DefineControllerType(string.Format("{0}.Home", mvcApplicationName), "BarController", new[] { "Get" });
        ...
        public void It_should_not_retrieve_the_controller_type()
        {
            ...
            var mvcApplicationName = Guid.NewGuid().ToString();
            var mvcApplication = MvcApplicationBuilder.Build(mvcApplicationName);
            mvcApplication.DefineControllerType(string.Format("{0}.Home", mvcApplicationName), "IndexController", new[] { "Get" });
            mvcApplication.DefineControllerType(string.Format("{0}.Home", mvcApplicationName), "FooController", new[] { "Get" });
            mvcApplication.DefineControllerType(string.Format("{0}.Home", mvcApplicationName), "BarController", new[] { "Get" });
